...
public class ContactDataGenerator {
    ...
    private void saveAsCsv(List<ContactData> contacts, File file) throws IOException {
        ...
        for (ContactData contact : contacts){
            writer.write(String.format("%s;%s;%s;%s;%s\n", contact.getUsername(), contact.getFamilyName(), contact.getAddress(), contact.getHomePhone(), contact.getEmail()));
        }
        writer.close();
    }
    ...
    private  List<ContactData> generateContacts(int count) {
        ...
        for (int i = 0; i < count; i++) {
            contacts.add(new ContactData().withUsername(String.format("Asya %s", i)).withFamilyName(String.format("Kas %s", i)).withAddress(String.format("address %s", i))
                    .withHomePhone(String.format("+7 495 111 11 1%s", i)).withEmail(String.format("a.kas%s@mail.ru", i)));
        }
        return contacts;
    }
